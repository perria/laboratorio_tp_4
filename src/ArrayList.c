...
int al_push(ArrayList* pList, int index, void* pElement)
{
    ...
    if(pList!=NULL && pElement!=NULL && (index>=0 && index<pList->size))
    {
        for(i=index; i<al_len(pList);i++)
        {
          pList->pElements[index]=pList->pElements[i+1];
          returnAux=0;
        }
...
ArrayList* al_subList(ArrayList* pList,int from,int to)
{
    ...
    if(pList!=NULL)
    {
        if(from>=0 && from<=pList->size && to>=from && to<=pList->size)
        {
                pListDos=al_newArrayList();

                if(pListDos!=NULL)
                {
                    for(i=from;i<to;i++)
                    {
                        al_add(pListDos,al_get(pList,i));
                    }
                    return pListDos;
                 }
         }
...
int al_containsAll(ArrayList* pList,ArrayList* pList2)
{
    ...
    if(pList!=NULL && pList2!=NULL)
    {
        for(j=0;j<al_len(pList2);j++)
        {
            for(i=0;i<al_len(pList);i++)
            {
                if(pList->pElements==pList2->pElements)
                {
                    cont++;
                    break;
                }
        ...
        for(i=0;i<al_len(pList)-1;i++)
        {
            for(j=i+1;j<al_len(pList);j++)
            {
                ...
                case -1:
                    ...
                    {
                        auxi=pList->pElements[i];
                        pList->pElements[i]=pList->pElements[j];
                        pList->pElements[j]=auxi;
                    }
                    break;
                case 0:
                    continue;
...
int resize_Down(ArrayList* pList)
{
